...
- if the bug is about Gemma chat-audio (lemonade-chat backend) → `transcribe/chat_audio.go` (chunking + POST + SSE + few-shot history) and `transcription.chat_audio` in the config
