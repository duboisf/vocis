...
- [`internal/transcribe/chat_audio.go`](/home/fred/git/vtt/internal/transcribe/chat_audio.go): the chat-audio dictation session — the only transcription backend vocis ships. Reads samples → Silero VAD cuts clips at pauses → at release one POST to `/chat/completions` with every clip as its own `input_audio` content part. SSE deltas drive overlay partials during Finishing. The dictation hits the model exactly once. Implements the `Dictation` interface so `app` and `recall` consume it identically.
...
- `transcribe.startChatAudioSession` owns Silero clip cutting and the single release-time `/chat/completions` POST whose text `Finalize` returns
...
- if the bug is about Gemma chat-audio (lemonade-chat backend) → `transcribe/chat_audio.go` (clip cutting + POST + SSE) and the energy-gate/silero knobs under `transcription.*` in the config
